...
const FormularioChurus = () => {
  const [stock, setStock] = useState(0)
  const [productoId, setProductoId] = useState(null)
  const [unidadesCompradas, setUnidadesCompradas] = useState('')
  ...
  const NOMBRE_BD = "churus"
  const NOMBRE_MOSTRAR = "churus tuna"
  ...
  const verificarTablaVentas = async () => {
    try {
      ...
      if (error) {
        ...
        console.log('⚠️ Posiblemente la tabla "ventas" no existe o no tiene RLS configurado')
      } else {
        console.log('✅ Tabla ventas verificada. Columnas disponibles:', data.length > 0 ? Object.keys(data[0]) : 'Sin datos')
      }
    } catch (err) {
  ...
  const cargarStock = async () => {
    ...
    if (data && data.length > 0) {
      ...
      console.log("✅ Producto encontrado - ID:", data[0].id, "Stock:", data[0].stock)
    } else {
  ...
  const handleForm = async (e) => {
    ...
    const unidadesNum = Number(unidadesCompradas)
    const precioNum = Number(precio)

    if (!unidadesCompradas || unidadesNum <= 0) {
      alert('❌ Ingresá una cantidad válida de unidades')
      return
    }
    ...
    const nuevoStock = stock - unidadesNum
    ...
    try {
      ...
      if (errorVenta) {
        ...
        if (errorVenta.code === '23502') {
          alert('❌ Error: La tabla ventas tiene una columna que no existe o es requerida')
        } else if (errorVenta.code === '23503') {
          alert('❌ Error: El producto_id no existe en la tabla productos o hay problema de clave foránea')
        } else if (errorVenta.code === '42501') {
          alert('❌ Error: No tenés permisos para insertar en la tabla ventas (RLS)')
        } else {
...
export default FormularioChurus
